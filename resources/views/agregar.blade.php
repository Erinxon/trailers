...
@section('content')
    <section class="container">
        <div class="row justify-content-center align-items-center vh-60">
            <div class="col-md-8 content-agregar">
                <div class="modal-body">
                    <form method="post" action="{{ route('admin.store') }}" id="formulario" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            ...
                            <div class="col-lg-6 mt-2">
                                <input type="text" name="year" class="form-control" placeholder="Año" value="{{ old('year') }}">
                                <span class="text-danger">{{$errors->first('year')}}</span>
                            </div>
                            <div class="col-lg-6 mt-2">
                                <input type="text" name="duracion" class="form-control" placeholder="Duración" value="{{ old('duracion') }}">
                                <span class="text-danger">{{$errors->first('duracion')}}</span>
                            </div>
                            <div class="col-lg-6 mt-2">
                                <input type="text" name="genero" class="form-control" placeholder="Género" value="{{ old('genero') }}">
                                <span class="text-danger">{{$errors->first('genero')}}</span>
                            </div>
                            <div class="col-lg-12 mt-2">
                                <textarea type="text" name="reparto" class="form-control" placeholder="Reparto">{{ old('reparto') }}</textarea>
                                <span class="text-danger">{{$errors->first('reparto')}}</span>
                            </div>
                            <div class="col-lg-12 mt-2">
                                <textarea type="text" name="Sinopsis" class="form-control" placeholder="Sinopsis">{{ old('Sinopsis') }}</textarea>
                                <span class="text-danger">{{$errors->first('Sinopsis')}}</span>
                            </div>
                            <div class="col-lg-12 mt-2">
                                <input type="text" name="url" class="form-control" placeholder="Url Video" id="urlCampo">
                        ...
                        <div class="modal-footer">
                            ...
                            <button class="btn btn-primary" id="guardar-trailer">Guardar
                                Trailer</button>
                        </div>
                    </form>
                </div>
